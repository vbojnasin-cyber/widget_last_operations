def get_mask_card_number(number_card: str) -> str:
    ...
    if number_card == " " or number_card == "":
        raise ValueError(f"Номер карты не может состоять из пустой строки, вы ввели - {number_card}")
    clean_number_card = number_card.replace(" ", "")
    if not clean_number_card.isdigit():
        raise ValueError(f"Номер карты должен состоять только из цифр, номер который вы ввели - {clean_number_card}")
...
def get_mask_account(number_account: str) -> str:
    ...
    if number_account == " " or number_account == "":
        raise ValueError(f"Номер счета не может состоять из пустой строки, вы ввели - {number_account}")
    clean_number_account = number_account.replace(" ", "")
    if not clean_number_account.isdigit():
        raise ValueError(f"Номер счета должен состоять только из цифр, номер который вы ввели - {clean_number_account}")
