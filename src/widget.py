...
def mask_account_card(number_account: str) -> str:
    ...
    if not isinstance(number_account, str):
        raise ValueError(f"Данные должны быть строкой, ваш тип данных - {type(number_account)}")
    ...
    if not number_card.isdigit():
        raise ValueError(f"Номер карты должен состоять только из цифр, ваш номер карты - {number_card}")
    try:
